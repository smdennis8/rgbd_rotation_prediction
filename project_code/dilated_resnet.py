...
class DRN(nn.Module):
    def __init__(
        self, block, layers, arch="D", channels=(16, 32, 64, 128, 256, 512, 512, 512), BatchNorm=None, input_channels=3
    ):
        ...
        if arch == "C":
            self.conv1 = nn.Conv2d(input_channels, channels[0], kernel_size=7, stride=1, padding=3, bias=False)
...
def drn_d_22(BatchNorm, pretrained=True, input_channels=8):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch="D", BatchNorm=BatchNorm, input_channels=input_channels)
    if pretrained:
        ...
        strict = True
        if input_channels != 3:
            # All model checkpoints assume 3-channel (RGB) input
            del pretrained["layer0.0.weight"]
            strict = False

        model.load_state_dict(pretrained, strict=strict)
    return model
...
def drn_d_54(BatchNorm, pretrained=True, input_channels=3):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch="D", BatchNorm=BatchNorm, input_channels=input_channels)
    if pretrained:
